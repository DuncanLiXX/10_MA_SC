...
void SetMcArmComm(MCArmCommunication *comm){this->m_p_mc_arm_comm = comm;}   //设置MC-ARM通讯接口
    ...
    bool ReadAlarm_pmc();                                //读取 PMC 报警配置
#else
    ...
    bool GetPhyAxistoChanAxis(uint8_t phy_axis, uint8_t &get_chan_id, uint8_t &get_chan_axis);  //根据物理轴号获取通道轴号
...
//	void SetAxisAccParam(uint8_t index);			//设置指定轴加速度相关信息
    ...
    void ManualMove(uint8_t phy_axis, int8_t dir, double vel, double inc_dis, bool ratio = true);  //手动移动，向dir方向移动dis距离
    void ManualMoveAbs(uint8_t phy_axis, double vel, double pos, bool ratio = true);   //手动以目标速度vel移动至绝对目标位置
    bool ManualMoveAbs(uint8_t phy_axis, double vel, double pos, double &real_pos, bool ratio = true);   //手动以目标速度vel移动至绝对目标位置
    ...
    bool SetPmcRetRef(uint8_t phy_axis, uint32_t feed_rate);     //设置PMC轴进入建立机械坐标系流程
    int32_t GetPmcReturnSpeed(int8_t axisid);//临时用，获取PMC轴回零速度
    ...
    double GetSpdAngleFeedback(uint8_t index){return m_df_spd_angle[index];}    //获取指定轴的主轴角度反馈
    double GetPmcAxisRemain(uint8_t index){return m_df_pmc_axis_remain[index];} //获取PMC轴余移动量反馈
    ...
    PmcAxisCtrl *GetChnPmcAxisCtrl(uint8_t chn_index); //获取指定通道的PMC轴控制组件
...
//	void SetAxisSoftLimitValue(uint8_t axis, uint8_t index);   //设置指定轴的软限位值
    ...
    void UpdateHandwheelState(uint8_t chn);   //设置MI模块手轮跟踪模式
    void SetMiHandwheelInsert(bool flag, uint8_t chn);  //设置MI模块手轮插入模式
    ...
    bool NotidyHmiAxisRefComplete(uint8_t phy_axis);    //通知HMI参考点建立状态发生改变
    ...
    PmcAxisCtrl *GetPmcAxisCtrl(){return m_pmc_axis_ctrl;} // 获取pmc轴控制模块
    SyncAxisCtrl *GetSyncAxisCtrl(){return  m_sync_axis_ctrl;} // 获取同步轴控制模块
    uint8_t GetMlkMask(){return m_MLK_mask;}    // 获取机械锁住标记
    double* GetMlkPos(){return m_MLK_pos;}      // 获取机械锁住位置

    void ProcessPMCProtect();   //处理程序保护
    void SetProgProtect(int level);     // 设置程序保护状态
    bool UpdateMcModel(const string &mcPath);

    void UpdatePSW();   //刷新位置开关信号
    bool HasMDIData();
    // 检测是否软限位超限
    // dir: 方向
    // phy_axis: 物理轴号 从0开始
    ...
    void ClearAxisRefEncoder(int axisID);                        // 清除绝对式零点编码器

    bool SetPmcActive(uint64_t phy_axis);   //激活PMC轴
    bool RstPmcActive(uint64_t phy_axis);   //取消激活PMC轴
    bool GetPmcActive(uint64_t phy_axis);   //PMC轴是否激活
    bool GetAxisRetRefFlag(uint64_t phy_axis);  //是否建立参考点

    int GetCurChannelIndex() { return m_n_cur_channle_index; }

    bool SetConsumeTask(TASK_CONSUME_TYPE consumeType); //激活后台线程
    ...
    void ProcessSAsingal(bool force = false);    //急停信号信号相关处理
...
private:	//私有成员函数
    ...
    void ProcessHmiMdiInfo(HMICmdFrame &cmd);           //处理HMI设置MDI相关信息
    ...
    void ProcessHmiAbsoluteRefSet(HMICmdFrame &cmd);    //处理HMI设置绝对式编码器原点
    ...
    void ProcessConsumingTask();     //梯图刷新耗时任务提取到此线程处理
...
//    void SendMiParam(uint8_t axis, uint32_t para_no, T data);  //发送MI参数
    ...
    void SetAxisComplete(int axisID);                            // 设置绝对式编码器有无回过零标记
    void GotoZeroPos(int axisID);                                // 运动到零点坐标
...
//#endif	
    double *m_df_spd_angle;     // 主轴当前角度反馈

    uint64_t m_n_phy_axis_svo_on;		//物理轴使能标志
    ...
    bool m_mdi_mode_ready;          //mdi准备好信号,界面点击载入按钮后，mdi才能运行测试,HMI点击载入会置位，清除会复位
    ...
    uint64_t m_pos_hard_limit_last{0};  //上一周期的硬限位标记
    uint64_t m_neg_hard_limit_last{0};  //上一周期的硬限位标记
...
//	uint64_t m_n_ret_ref_encoder;   //粗基准编码器值
    ...
    uint64_t m_pmc_axis_active_mask;    //已激活的PMC轴
    ...
    bool m_servo_ready{false};     //伺服是否初始化完成（全部轴上使能就算完成）
    uint8_t m_cur_svf{0x00};      //当前的伺服关断标记
