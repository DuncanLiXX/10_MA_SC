...
bool ChannelControl::Initialize(uint8_t chn_index, ChannelEngine *engine, HMICommunication *hmi_comm,
	...
	// 读取自定义步长
	fp2 = fopen("/cnc/config/custom_step_inc", "rb");

	if(fp2 != NULL){
		fread(custom_step_inc, 1, sizeof(double)*32, fp2);
		fclose(fp2);
	}
...
//    }
    ...
    if(m_order_step == STEP_PREPROG || m_order_step == STEP_ENDPROG){

    	if(order_file_vector.empty()){
    		char path[kMaxPathLen] = {0};
    		strcpy(path, PATH_NC_FILE);
			strcat(path, reset_file);
			strcpy(m_channel_status.cur_nc_file_name, reset_file);
...
void ChannelControl::saveBreakPoint(bool force_save){
    ...
    if(m_channel_status.chn_work_mode == AUTO_MODE  &&
        ...
        if(m_n_subprog_count > 0 || m_n_macroprog_count > 0)
            brk_line_number = 0; // 在子程序中无法记录断点
...
bool ChannelControl::SetSysVarValue(const int index, const double &value){
    ...
    if(index >= 2001 && index <= 2128){   //刀长磨损补偿
        ...
        if(id < m_p_channel_config->tool_number){
            ...
            this->m_p_chn_tool_config->length_wear_comp[id] = value;
        ...
        if(id < m_p_channel_config->tool_number){
            ...
            this->m_p_chn_tool_config->length_compensation[id] = value;
            g_ptr_parm_manager->UpdateToolMeasure(m_n_channel_index, id, value);
            this->NotifyHmiToolOffsetChanged(id+1);   //通知HMI刀偏值更改
        ...
        if(value < m_p_channel_config->tool_number){
            this->m_channel_status.preselect_tool_no = value;
        }else
            return false;

    }else if(index >= 4322 && index <= 4336){
        this->m_preselect_tool_arr[index-4321] = value;
    }
    else if(index >= 5201 && index <= 5220){   //基本坐标系偏移
        int id = index-5201;
        if(id < this->m_p_channel_config->chn_axis_count){
        ...
        if(id < m_p_channel_config->tool_number){
            ...
            return false;
    }else if(index >= 24001 && index <= 24128){
        int id = index - 24001;
        if(id < m_p_channel_config->tool_number){
            this->m_p_chn_tool_config->geometry_compensation[id][0] = value;
            g_ptr_parm_manager->UpdateGeoComp(m_n_channel_index, id, 0, value);
            this->NotifyHmiToolOffsetChanged(id+1);   //通知HMI刀偏值更改
        }
        else
            return false;
    }else if(index >= 24201 && index <= 24328){
        int id = index - 24201;
        if(id < m_p_channel_config->tool_number){
            this->m_p_chn_tool_config->geometry_compensation[id][1] = value;
            g_ptr_parm_manager->UpdateGeoComp(m_n_channel_index, id, 1, value);
            this->NotifyHmiToolOffsetChanged(id+1);   //通知HMI刀偏值更改
        }
        else
            return false;
    }else if(index >= 24401 && index <= 25128){
        int id = index - 24401;
        if(id < m_p_channel_config->tool_number){
            this->m_p_chn_tool_config->geometry_compensation[id][2] = value;
            g_ptr_parm_manager->UpdateGeoComp(m_n_channel_index, id, 2, value);
            this->NotifyHmiToolOffsetChanged(id+1);   //通知HMI刀偏值更改
        }
        else
            return false;
...
//	}
    ...
    if(m_p_g_reg->BRKC){
        if(strcmp(m_channel_status.cur_nc_file_name, brk_file_name) != 0 || brk_line_number == 0){
//			m_error_code = ERR_AXIS_REF_NONE;
//			CreateError(m_error_code, ERROR_LEVEL, CLEAR_BY_MCP_RESET, 0, 0, 0);
//			return;
        }else{
            // 暂停后断点继续 不用程序再起
            if(m_channel_status.machining_state != MS_PAUSED && m_channel_status.chn_work_mode == AUTO_MODE){
...
END:
    ...
    if(m_p_channel_config->order_prog_mode == 4 &&
		m_eliminate_step == 0 &&
    ...
    }else if((m_p_channel_config->order_prog_mode == 1 ||m_p_channel_config->order_prog_mode == 3) &&
    		m_eliminate_step == 0 &&
    		!m_b_in_common_pre_prog &&
			 m_n_restart_mode == NOT_RESTART&&
    		 m_channel_status.chn_work_mode == AUTO_MODE){
        CallMacroProgram(m_p_channel_config->pre_prog_num);
    }
...
void ChannelControl::StartMcIntepolate(){
    ...
    mc_work_mode = MC_MODE_AUTO;  //MDA 模式与auto合并
...
/**
 * @brief 判断子程序类型是否为当前程序所在目录
 * @param type
 * @return
 */
bool ChannelControl::SubProgIsCurDir(uint8_t type)
{
    // 没找到对应子程序   1--在本程序内   2--同目录下nc文件    3--系统子程序目录nc文件    4--同目录下iso文件    5--系统子程序目录iso文件
...
void ChannelControl::ProcessHmiCmd(HMICmdFrame &cmd){
    ...
    case CMD_HMI_CLEAR_TOOL_COMP:        // 清除刀具补偿
        this->ProcessHmiClearToolComp(cmd);
    	break;
    case CMD_HMI_CLEAR_TOOL_OFFSET:      // 清除刀具偏置
        this->ProcessHmiClearToolOffset(cmd);
        break;
    ...
    case CMD_HMI_SET_SYSTEM_MCODE:
        this->ProcessHmiSetMcodeSystem(cmd);
        break;
...
void ChannelControl::ProcessHmiSetCustomStepInc(HMICmdFrame &cmd){
	...
	if(axis >= 0 && axis < 32){
		...
		FILE * fp = fopen("/cnc/config/custom_step_inc", "wb");

		if(fp!= NULL){
			fwrite(custom_step_inc, sizeof(double)*32, 1, fp);
			fclose(fp);
		}
...
void ChannelControl::ProcessHmiClearToolComp(HMICmdFrame &cmd)
{
    int index, count;
    ...
    if(index > 0 && index < kMaxToolCount &&
       (index+count) > 0 && (index+count) <= kMaxToolCount+1){

        printf("ProcessHmiClearToolComp start: %d count: %d\n", index, count);

        g_ptr_parm_manager->ClearToolComp(0, index, count);

        if(this->m_channel_status.gmode[8] != G49_CMD)
            this->SetMcToolOffset(true);
...
void ChannelControl::ProcessHmiClearToolOffset(HMICmdFrame &cmd)
{
    int index, count;
    ...
    if(index > 0 && index < kMaxToolCount &&
       (index+count) > 0 && (index+count) <= kMaxToolCount+1){
        ...
        g_ptr_parm_manager->ClearToolOffset(0, index, count);
 ...
 */
bool ChannelControl::DoRestart(uint64_t line_no){
    ...
    //切换主轴状态  恢复主轴转速
    if(m_p_spindle->GetSpindleSpeed() != this->m_mode_restart.cur_scode){  //指定速度不同
        msg = new SpeedMsg(m_mode_restart.cur_scode);
        if(msg != nullptr){
            msg->SetLineNo(line_no);
...
bool ChannelControl::SendMDIOpenFileCmdToHmi(char *filename)
{
    ...
    std::cout << "------------> SendMDIOpenFileCmdToHmi: " << cmd.data << " " << (int)strlen(cmd.data) << std::endl;
...
void ChannelControl::SetMachineState(uint8_t mach_state){
    ...
    if(m_channel_status.chn_work_mode == AUTO_MODE
		&& m_channel_status.machining_state == MS_RUNNING
		&& (mach_state == MS_WARNING ||mach_state == MS_PAUSING
...
uint8_t ChannelControl::GetWorkMode(){
	return m_channel_status.chn_work_mode;
}
...
int ChannelControl::Run(){
    ...
    RunStep run_step = STEP_GETLINE;  // 编译执行步骤
    ...
    while(!g_sys_state.system_quit)
    {
        ...
        if(m_n_run_thread_state == RUN)
        {
				...
				if(!m_p_compiler->CompileLine())  //编译一行代码
				{
					...
					if(m_p_compiler->RunMessage()){
						...
						if(m_p_compiler->GetErrorCode() != ERR_NONE){
							CreateError(m_p_compiler->GetErrorCode(), ERROR_LEVEL, CLEAR_BY_MCP_RESET, m_p_compiler->GetCurLine(), m_n_channel_index);
							m_n_run_thread_state = ERROR;
						}else{
            ...
            if(m_p_compiler->IsEndOfFile() && !m_p_compiler->IsCompileOver() && !m_p_compiler->IsPreScaning()
                    && this->m_channel_status.chn_work_mode == AUTO_MODE /*&& !m_p_compiler->IsSubProgram()*/
                    )
            {

                if(!b_in_end_prog){  //调用后置程序时 禁用此报错
                    m_n_run_thread_state = ERROR;
        ...
        else
        {
            ...
            if( m_p_channel_config->order_prog_mode == 4 &&
        		...
        		}else if(m_order_step == STEP_ENDPROG){
                    if (IsStepMode())
                    {
                        this->SetMcStepMode(true);
                        m_b_need_delay_step = false;
				...
				if(++m_n_step_change_mode_count > check_count){
					...
					//	printf("change to pause in RefreshStatusFun();change_to_pause=%hhu\n", this->m_b_need_change_to_pause);
						if(IsStepMode()){
                            ...
                            {
                                std::cout << "RefreshStatusFunc --> m_b_need_change_to_pause" << std::endl;
								this->m_b_need_change_to_pause = true;
                            }
						}
...
bool ChannelControl::IsNcFileExist(char *file_name){
	...
	//if(strcmp("O9000.NC", file_name) == 0) return true;
...
bool ChannelControl::IsStepMode(){
    if(m_channel_status.func_state_flags.CheckMask(FS_SINGLE_LINE) &&     //单段有效
            (m_channel_status.chn_work_mode == AUTO_MODE || m_channel_status.chn_work_mode == MDA_MODE)&&               //自动模式
            (m_simulate_mode == SIM_NONE)   //非仿真状态
            && !m_b_dust_eliminate   //非除尘状态
            ){
	...
	if(gcode == G92_CMD){ //设定工件坐标系
		...
		for(int i=0; i<kMaxAxisChn; i++){
			...
			if(coord_index <= G59_CMD ){
				offset -= m_p_chn_coord_config[coord_index/10-53].offset[i];
			}else if(coord_index <= G5499_CMD){
				offset -= m_p_chn_ex_coord_config[coord_index/10-5401].offset[i];
			}


            uint16_t g_compensation = this->m_channel_status.gmode[8];
            if(g_compensation == G43_CMD && i < 3){
                offset -= m_p_chn_tool_config->geometry_compensation[m_channel_status.cur_h_code-1][i];
                offset -= m_p_chn_tool_config->geometry_wear[m_channel_status.cur_h_code-1][i];
                if(i==2){
                    offset -= m_p_chn_tool_config->length_compensation[m_channel_status.cur_h_code-1];
                    offset -= m_p_chn_tool_config->length_wear_comp[m_channel_status.cur_h_code-1];
                }
            }else if(g_compensation == G44_CMD && i<3){
                offset += m_p_chn_tool_config->geometry_compensation[m_channel_status.cur_h_code-1][i];
                offset += m_p_chn_tool_config->geometry_wear[m_channel_status.cur_h_code-1][i];
                if(i==2){
                    offset += m_p_chn_tool_config->length_compensation[m_channel_status.cur_h_code-1];
...
//			int phy_axis = this->GetPhyAxis(i);
//			if(phy_axis != 0xFF){
...
//			}
	...
	if(gcode == G52_CMD){//局部坐标系
        ...
        if(G52Active){
            ...
            for(int i=0; i<8; ++i) G52offset[i] = 0;
...
bool ChannelControl::ExecuteSpeedMsg(RecordMsg *msg){
    ...
    if(this->m_n_restart_mode != NOT_RESTART &&
        ...
        this->m_mode_restart.cur_scode = int(speed->GetSpeed());
    ...
    if(!this->IsStepMode() && ((m_channel_rt_status.line_no+forward_line) >= msg->GetLineNo())){  //非单段模式下的主轴启动指令预启动
        ...
        while(count < 4 ){
            ...
            if(msg->GetLineNo() == m_channel_mc_status.cur_line_no){
                break;
            }
...
bool ChannelControl::ExecuteCompensateMsg(RecordMsg *msg){
    ...
    if(type == G43_CMD || type == G44_CMD || type == G49_CMD){//G43/G44    即时更新到通道状态的模态信息中
    	...
    	if(type == G44_CMD){
    		g44_active = true;
    	}else if(type == G43_CMD || type == G49_CMD){
    		g44_active = false;
    	}
...
//            			m_channel_rt_status.tar_pos_work = m_channel_mc_status.intp_tar_pos;
        ...
        case 0://第一步：将新偏置发送到MC
            ...
            z_axis_offset += m_p_chn_tool_config->geometry_wear[value-1][2] * 1e3;   //叠加磨损补偿
...
bool ChannelControl::ExecuteRefReturnMsg(RecordMsg *msg){
    ...
    if(this->m_n_restart_mode != NOT_RESTART &&
    		main_prog_line_number < this->m_n_restart_line-1
        #ifdef USES_ADDITIONAL_PROGRAM
            && this->m_n_add_prog_type == NONE_ADD      //非附加程序运行状态
        #endif
            ){//加工复位
        ...
        if(gcode == G28_CMD){
        	...
        	for(int i = 0; i < this->m_p_channel_config->chn_axis_count; i++){
				if(axis_mask & (0x01<<i)){
					pos.m_df_point[i] = m_p_axis_config[this->GetPhyAxis(i)].axis_home_pos[ref_id-1];
				}
			}
    ...
    //this->TransMachCoordToWorkCoord(mid_pos, m_mode_restart.gmode[14], m_mode_restart.cur_h_code, axis_mask);
    ...
    if(gcode == G28_CMD or gcode == G30_CMD){
        ...
        {
            //第一步：控制对应的轴走到中间点位置
            DPointChn point = this->m_channel_rt_status.cur_pos_work;

            for(i = 0; i < m_p_channel_config->chn_axis_count; i++){
                if(axis_mask & (0x01<<i)){
                    point.SetAxisValue(i, pos[i]);
                }
            }

            StraightTraverse(0, point);
        ...
        case 2:
        {
            ...
            if(gcode == G28_CMD){
                for(i = 0; i < m_p_channel_config->chn_axis_count; i++){
                    if(axis_mask & (0x01<<i)){
                        double ppos = m_p_axis_config[i].axis_home_pos[0];
                        TransMachCoordToWorkCoord(ppos, m_channel_status.gmode[14], i);
                        point.SetAxisValue(i, ppos);
                    }
                }
                ...
                for(i = 0; i < m_p_channel_config->chn_axis_count; i++){
                    if(axis_mask & (0x01<<i)){
                        double ppos = m_p_axis_config[i].axis_home_pos[ref_id - 1];
                        TransMachCoordToWorkCoord(ppos, m_channel_status.gmode[14], i);
                        point.SetAxisValue(i, ppos);
                    }
                }
...
bool ChannelControl::ExecuteInputMsg(RecordMsg * msg){
    ...
    if(this->m_n_restart_mode != NOT_RESTART &&
    	...
    	int count = 0;
		while(count < 4 ){
            //printf("movecount: %d blockover: %d\n", this->ReadMcMoveDataCount(), this->CheckBlockOverFlag());
            if(this->ReadMcMoveDataCount() > 0 || !this->CheckBlockOverFlag() ||
					m_channel_status.machining_state == MS_PAUSED ||
					m_channel_status.machining_state == MS_WARNING){
				return false;    //还未运行到位
    ...
    case 10:{
    	...
    	if(isAbs){
            ...
            comp_data = this->m_p_chn_tool_config->length_compensation[tool_number-1] + input_msg->RData;
    	}
    	// 这里也会改变一次
        g_ptr_parm_manager->UpdateToolMeasure(this->m_n_channel_index, tool_number-1, comp_data);

        if(this->m_channel_status.gmode[8] != G49_CMD && this->m_channel_status.cur_h_code == tool_number)
            SetMcToolOffset(true);
    ...
    case 12:{
    	if(isAbs)
            this->m_p_chn_tool_config->length_wear_comp[tool_number-1] = input_msg->RData;
    	else
            this->m_p_chn_tool_config->length_wear_comp[tool_number-1] += input_msg->RData;

        double comp_data = this->m_p_chn_tool_config->length_wear_comp[tool_number-1];
...
bool ChannelControl::ExecuteOpenFileMsg(RecordMsg *msg){
	...
	if(index <= 0 || index > order_file_vector.size()){
        ...
        CreateError(ORDER_INDEX_ERROR, ERROR_LEVEL, CLEAR_BY_MCP_RESET, openfile_msg->GetLineNo(), m_n_channel_index, 0xff);
		return false;
	}
...
void ChannelControl::resetEliminate()
{
    char path[kMaxPathLen] = {0};
    strcpy(path, PATH_NC_FILE);
    strcat(path, eliminate_breakfile);
...
bool ChannelControl::GetLimitTargetPos(ManualMoveDir dir, uint8_t chn_axis, int64_t &tar_pos)
{
    ...
    if(chn_axis < 3){
        ...
        if(this->m_channel_status.gmode[8] == G43_CMD){
            offset += m_p_chn_tool_config->geometry_compensation[m_channel_status.cur_h_code-1][chn_axis];
            offset += m_p_chn_tool_config->geometry_wear[m_channel_status.cur_h_code-1][chn_axis];
        ...
        else if(this->m_channel_status.gmode[8] == G44_CMD){
            offset -= m_p_chn_tool_config->geometry_compensation[m_channel_status.cur_h_code-1][chn_axis];
            offset -= m_p_chn_tool_config->geometry_wear[m_channel_status.cur_h_code-1][chn_axis];
...
void ChannelControl::ManualMove(int8_t dir){
    ...
    // 同步轴检测
    if(m_p_axis_config[phy_axis].sync_axis){
        int grp_index = 0;
        bool flag_find =  false;

        vector<vector<uint8_t>> &sync_axis = m_p_channel_engine->GetSyncAxis();

        for(vector<uint8_t> l : sync_axis){
            for(uint8_t axis: l){
                if(axis == l.front()) continue;
                if(axis == phy_axis){
                    flag_find = true;
                    break;
                }
...
void ChannelControl::SetMcStepMode(bool flag){
    ...
    printf("ChannelControl::SetMcStepMode flag: %d\n", flag);
    McCmdFrame cmd;
    memset(&cmd, 0x00, sizeof(McCmdFrame));
...
void ChannelControl::SetMcAxisToolOffset(uint8_t axis_index){
    ...
    if(axis_config.axis_linear_type == LINE_AXIS_Z){
        ...
        printf("geocomp: %lf geowear: %lf lencomp: %lf lenwear: %lf\n",
               m_p_chn_tool_config->geometry_compensation[m_channel_status.cur_h_code-1][2],
               m_p_chn_tool_config->geometry_wear[m_channel_status.cur_h_code-1][2],
               m_p_chn_tool_config->length_compensation[m_channel_status.cur_h_code-1],
               m_p_chn_tool_config->length_wear_comp[m_channel_status.cur_h_code-1]);

        offset = m_p_chn_tool_config->geometry_compensation[m_channel_status.cur_h_code-1][2] * 1e7;  //单位由mm转换为0.1nm
        offset += m_p_chn_tool_config->geometry_wear[m_channel_status.cur_h_code-1][2] * 1e7;   //叠加磨损补偿
        offset += m_p_chn_tool_config->length_compensation[m_channel_status.cur_h_code-1] * 1e7;
        offset += m_p_chn_tool_config->length_wear_comp[m_channel_status.cur_h_code-1] * 1e7;
    }
    else if(axis_config.axis_linear_type == LINE_AXIS_X || axis_config.axis_linear_type == LINE_AXIS_Y){
        printf("geocomp: %lf geowear: %lf",
               m_p_chn_tool_config->geometry_compensation[m_channel_status.cur_h_code-1][axis_config.axis_linear_type-1],
               m_p_chn_tool_config->geometry_wear[m_channel_status.cur_h_code-1][axis_config.axis_linear_type-1]);
        ...
        offset += m_p_chn_tool_config->geometry_wear[m_channel_status.cur_h_code-1][axis_config.axis_linear_type-1] * 1e7;   //叠加磨损补偿
    }
    else //非XYZ轴没有刀具偏置
    {
    	printf("非XYZ轴没有刀具偏置\n");
...
bool ChannelControl::CheckBlockOverFlag(){
    ...
    /*
    if(m_channel_status.chn_work_mode == AUTO_MODE || m_channel_status.chn_work_mode == MDA_MODE ||
            m_channel_status.chn_work_mode == MANUAL_MODE
        #ifdef USES_ADDITIONAL_PROGRAM
            && this->m_n_add_prog_type != CONTINUE_START_ADD
        #endif
            ){
    ...
    if(!this->m_b_mc_on_arm)
        m_channel_mc_status.mda_block_over = m_p_mc_comm->ReadAutoBlockRunOverFlag(m_n_channel_index);
    else
        m_channel_mc_status.mda_block_over = m_p_mc_arm_comm->ReadAutoBlockRunOverFlag(m_n_channel_index);
    return m_channel_mc_status.mda_block_over;
}
...
void ChannelControl::SetMCHandleSpeed()
{
    McCmdFrame cmd;
    memset(&cmd, 0x00, sizeof(McCmdFrame));
    ...
    cmd.data.channel_index = m_n_channel_index;
    cmd.data.axis_index = 0xff;
    //cmd.data.cmd = CMD_MC_SET_MPG_SPEED;

    m_p_mc_comm->WriteCmd(cmd);
}

void ChannelControl::SetMcToolOffsetReal(){
	...
	for(int i = 0; i < m_p_channel_config->chn_axis_count; i++){
		...
		if(i<3){
			...
			origin_pos += m_p_chn_tool_config->geometry_compensation[m_channel_status.cur_h_code-1][i];
			origin_pos += m_p_chn_tool_config->geometry_wear[m_channel_status.cur_h_code-1][i];
...
{
    printf("===== ChannelControl::UpdateAndLogWorkFile time: %d\n",m_time_curfile_maching);
...
void ChannelControl::clear_mda_data()
{
    this->m_p_output_msg_list->Clear();	//清空缓冲数据
    this->InitMcIntpMdaBuf();  //清空MC中的MDA数据缓冲
...
void ChannelControl::SendMcHandleSimSpeed()
{
    ...
    uint32_t data = m_p_channel_config->handle_sim_speed*1000/60;
    cmd.data.data[1] = data&0xFFFF;
    cmd.data.data[2] = (data>>16)&0xFFFF;
...
void ChannelControl::TransMachCoordToWorkCoord(DPointChn &pos, uint16_t coord_idx, uint32_t axis_mask){
	...
	printf("TransMachCoordToWorkCoord 1\n");
...
void ChannelControl::TransMachCoordToWorkCoord(DPointChn &pos, uint16_t coord_idx, uint16_t h_code, uint32_t axis_mask){
	...
	printf("TransMachCoordToWorkCoord 2\n");
...
void ChannelControl::TransMachCoordToWorkCoord(DPoint &pos, uint16_t coord_idx, uint32_t axis_mask){
	...
	printf("TransMachCoordToWorkCoord 3\n");
...
void ChannelControl::TransWorkCoordToMachCoord(DPointChn &pos, uint16_t coord_idx, uint32_t axis_mask){
    printf("TransWorkCoordToMachCoord 1\n");
	double origin_pos = 0;
    double *pp = pos.m_df_point;
    uint8_t phy_axis = 0xFF;
...
void ChannelControl::TransWorkCoordToMachCoord(DPoint &pos, uint16_t coord_idx, uint32_t axis_mask){
    printf("TransWorkCoordToMachCoord 2\n");
	double origin_pos = 0;
    double *pp = &pos.x;
    uint8_t phy_axis = 0xFF;
...
void ChannelControl::TransMachCoordToWorkCoord(double &pos, uint16_t coord_idx, uint8_t axis){
	...
	printf("TransMachCoordToWorkCoord 4\n");
...
void ChannelControl::TransWorkCoordToMachCoord(double &pos, uint16_t coord_idx, uint8_t axis){
	...
	printf("TransWorkCoordToMachCoord 3\n");
    printf("===== start: %lf axis: %d\n", pos, axis);
...
void ChannelControl::TransWorkCoordToMachCoord(double &pos, uint16_t coord_idx, uint16_t h_code, uint8_t axis){
	...
	printf("TransWorkCoordToMachCoord 4\n");
                ...
                if(phy_axis != 0xFF){
                    if(this->m_p_axis_config[phy_axis].axis_linear_type == LINE_AXIS_Z && m_channel_status.cur_h_code > 0){//Z轴需要加上刀长偏置
                        ...
                        if (g_compensation == G44_CMD)
                        {
                            ...
                            origin_pos -= m_p_chn_tool_config->geometry_wear[m_channel_status.cur_h_code-1][2];   //叠加磨损补偿
                        }
                        else
                        {//G43_CMD G49_CMD
                            origin_pos += m_p_chn_tool_config->geometry_compensation[m_channel_status.cur_h_code-1][2];  //
...
void ChannelControl::StraightTraverse(int chn, DPointChn point)
{
    GCodeFrame frame;
    memset(&frame, 0x0, sizeof(frame));
    frame.data.frame_index = 0;
    frame.data.cmd = MOVE_G00;
    frame.data.ext_type = 0x21;
    frame.data.ext_type |= 0x01;
    frame.data.ext_type |= 0x02;
    frame.data.ext_type |= 0x20;

    frame.data.feed = 0;

    frame.data.pos0 = MM2NM0_1(point.m_df_point[0]); //单位由mm转换为0.1nm
    frame.data.pos1 = MM2NM0_1(point.m_df_point[1]);
    frame.data.pos2 = MM2NM0_1(point.m_df_point[2]);
    frame.data.pos3 = MM2NM0_1(point.m_df_point[3]);
    frame.data.pos4 = MM2NM0_1(point.m_df_point[4]);
    frame.data.pos5 = MM2NM0_1(point.m_df_point[5]);
    frame.data.pos6 = MM2NM0_1(point.m_df_point[6]);
    frame.data.pos7 = MM2NM0_1(point.m_df_point[7]);
