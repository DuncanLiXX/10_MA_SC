...
static bool need_sync_reset = false;
static bool sync_axis_ready = true;
...
void ChannelEngine::InitSyncAxisData()
{
    for(int i=0; i< m_p_channel_config[0].chn_axis_count; i++){

        //是同步轴
        if(m_p_axis_config[i].sync_axis == 1){

            bool flag_find = false;

            for(vector<uint8_t> &list : m_sync_axis_list){
                if(list.front() == m_p_axis_config[i].master_axis_no-1){
                    list.push_back(i);
                    flag_find = true;
                    continue;
                }
            ...
            if(flag_find == false){
                vector<uint8_t> l;
                l.push_back(m_p_axis_config[i].master_axis_no-1);
                l.push_back(i);
                m_sync_axis_list.push_back(l);
            }
...
void ChannelEngine::ProcessMiEnSyncAxisRsp(MiCmdFrame &cmd){
    int axisId = cmd.data.data[0];
    bool enable = cmd.data.data[1];
    bool success = (cmd.data.data[2]==0);
    m_sync_axis_ctrl->RspEnSyncAxis(axisId, enable,success);
}
...
void ChannelEngine::ProcessMiSyncAxis(MiCmdFrame &cmd){
    ...
    sync_axis_ready = true;
    printf("===== sync axis ready\n");
    //int64_t mask = 0;
...
void ChannelEngine::ProcessMiBusError(MiCmdFrame &cmd){
    ...
    printf("receive Mi Bus err: no=%hhu, sub_idx=%hhu, idx=%hu, code=%hu\n",
           slave_no, err_sub_index, err_index, err_code);
...
void ChannelEngine::ProcessHmiAbsoluteRefSet(HMICmdFrame &cmd)
{
    ...
    {
        if (errCode == 0)
            CreateError(ERR_RET_REF_FAILED, ERROR_LEVEL, CLEAR_BY_MCP_RESET, 0, m_n_cur_channle_index, chn_axis);
        else if(errCode == 1)
...
void ChannelEngine::ProcessPmcRefRet(uint8_t phy_axis){
    ...
    if(g_ptr_alarm_processor->HasErrorInfo()){ //有告警，拒绝回零
        ...
        CreateError(ERR_RET_REF_FAILED, ERROR_LEVEL, CLEAR_BY_MCP_RESET, 0, chan_id, axis_id);
        return;
    }
...
void ChannelEngine::ManualMove(uint8_t phy_axis, int8_t dir, double vel, double inc_dis, bool ratio){
    ...
    if(m_p_axis_config[phy_axis].sync_axis){
        int grp_index = 0;
        bool flag_find =  false;
        for(vector<uint8_t> l : m_sync_axis_list){
...
bool ChannelEngine::CheckAxisHardLimit(uint8_t phy_axis, int8_t dir){
    ...
    for(vector<uint8_t> l: m_sync_axis_list){
        ...
        if((g_reg->SYNC & (1<<grp_index)) == 0) continue;
        ...
        if(l.front() == phy_axis){
            ...
            for(uint8_t axis: l){
                if(axis == phy_axis) continue;

                if(dir == DIR_POSITIVE){
                    if(this->m_hard_limit_postive & (1<<axis))
                        return true;
                }else if(dir == DIR_NEGATIVE){
                    if(this->m_hard_limit_negative & (1 << axis))
                        return true;
                }
            }
        }
...
void ChannelEngine::SystemReset(){
    ...
    if(need_sync_reset){
        need_sync_reset = false;
        sync_axis_ready = false;
        printf("===== sync axis not ready\n");
...
void ChannelEngine::EcatIncAxisFindRefWithZeroSignal(uint8_t phy_axis){
    ...
    case 9:{ //等待Z信号捕获完成或者移动到位捕获失败，在Mi指令响应处理函数中处理
        if(fabs(this->GetPhyAxisMachPosFeedback(phy_axis) - m_df_ret_ref_tar_pos[phy_axis]) <= 0.010){  //运行到位还是未找到Z信号
            ...
            m_n_ret_ref_step[phy_axis] = 21;   //跳转失败处理
        }
    }
        break;
    ...
    case 17:
        ...
        if(fabs(this->GetPhyAxisMachPosFeedback(phy_axis) - dis) <= 0.010){  //到位
            ...
            m_n_ret_ref_step[phy_axis]++;  //跳转下一步
            std::cout << "step 17" << this->GetPhyAxisMachPosFeedback(phy_axis) << " " << dis << std::endl;
            gettimeofday(&this->m_time_ret_ref[phy_axis], NULL);
    ...
    case 21:
    {
        ...
        uint8_t chan_id = CHANNEL_ENGINE_INDEX, axis_id = NO_AXIS;
        GetPhyAxistoChanAxis(phy_axis, chan_id, axis_id);
        CreateError(ERR_RET_REF_Z_ERR, ERROR_LEVEL, CLEAR_BY_MCP_RESET, 0, chan_id, axis_id);
        break;
    }
    default:
        break;
...
void ChannelEngine::PulseAxisFindRefNoZeroSignal(uint8_t phy_axis){
    ...
    case 8: //等待设置完成，移动到轴参考点1
        ...
        dis = this->m_p_axis_config[phy_axis].axis_home_pos[0];   // 原点坐标
#endif
        printf("20231115  ChannelEngine::PulseAxisFindRefNoZeroSignal : %lf\n", dis);
        this->ManualMove(phy_axis, dis, m_p_axis_config[phy_axis].ret_ref_speed, 0);
        m_n_ret_ref_step[phy_axis] = 9;  //跳转下一步
        printf("return ref, goto step 9\n");
        break;
...
void ChannelEngine::ReturnRefPoint(){
    ...
    for(uint i = 0; i < this->m_p_general_config->axis_count; i++){
        ...
        // 同步轴检测
        if(m_p_axis_config[i].sync_axis){
            int grp_index = 0;
            bool flag_find =  false;
...
void ChannelEngine::ProcessAxisDownSafe(){
    while(1){
        this->m_p_mi_comm->ReadPhyAxisCurFedBckPos(m_df_phy_axis_pos_feedback, m_df_phy_axis_pos_intp, m_df_phy_axis_speed_feedback,
            m_df_phy_axis_torque_feedback, m_df_spd_angle, m_p_general_config->axis_count);
        bool bReady = true;
    ...
    for(int i=0; i<m_p_channel_config->chn_axis_count; i++){
        m_p_mi_comm->SendAxisEnableCmd(i+1, false);
    }
...
void ChannelEngine::ProcessSAsingal(bool force)
{
    ...
    printf("ProcessSA Signal emergency: %d\n", m_b_emergency);
    ...
    while (m_b_emergency || force)
    {
        ...
        for(uint8_t i = 0; i < this->m_p_general_config->axis_count; i++){
            ...
            if (fabs(m_df_phy_axis_speed_feedback[i]) > this->m_p_axis_config[i].move_pr * 1000 * 100 / 60)
            {
                bReady = false;
                break;
            }
