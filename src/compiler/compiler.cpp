...
/**
 * @brief 获取子程序的路径数据
 * @param macro_group : 程序类型，1--在本程序内   2/6--同目录下nc文件    3/7--系统子程序目录nc文件    4/8--同目录下iso文件    5/9--系统子程序目录iso文件
 * @param macro_index : 程序号
 * @param abs_path : 是否绝对路径
 * @param name[out] : 输出程序文件路径
 */
void Compiler::GetMacroSubProgPath(int macro_group, int macro_index, bool abs_path, char *name){
    if(abs_path){
        ...
        if(macro_group == 2){//同目录下用户宏程序
            if(macro_index <= 9999)
                sprintf(name, "%sO%04d.nc", dirName.c_str(), macro_index);   //拼接文件名称
            else
            ...
            if(macro_index <= 9999)
                sprintf(name, "%sO%04d.iso", dirName.c_str(), macro_index);   //拼接文件名称
            else
            ...
            if(macro_index <= 9999)
                ...
                sprintf(name, "%sO%d.ISO", dirName.c_str(), macro_index);//拼接文件名称
 ...
 */
int Compiler::FindSubProgram(int sub_name, bool file_only, uint8_t scanMode) {   //查找并打开子程序
    ...
    if (scanMode == 1 && dirName != PATH_NC_SUB_FILE && dirName != PATH_NC_MAC_FILE)
    {
        ...
        if (sub_name <= 9999)
            ...
            sprintf(filepath, "%sO%d.nc", dirName.c_str(), sub_name);   //拼接文件绝对路径
        ...
        memset(filepath, 0x00, kMaxPathLen);
        if (sub_name <= 9999)
            sprintf(filepath, "%sO%04d.iso", dirName.c_str(), sub_name);   //拼接文件绝对路径
        else
            sprintf(filepath, "%sO%d.iso", dirName.c_str(), sub_name);   //拼接文件绝对路径
